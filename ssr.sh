...
BBR_installation_status(){
	if [[ ! -e ${BBR_file} ]]; then
		...
		cd "${file}"
		if ! wget -N --no-check-certificate https://blog.xbbai.site/ssr/bbr.sh; then
...
ss_link_qr(){
	...
	SSurl="ss://${SSbase64}"
SSQRcode="http://pan.baidu.com/share/qrcode?w=150&h=150&url=${SSurl}"
	ss_link=" SSR    链接 : ${Green_font_prefix}${SSurl}${Font_color_suffix} \n SS  二维码 : ${Green_font_prefix}${SSQRcode}${Font_color_suffix}"
}
ssr_link_qr(){
	...
	SSRQRcode="http://pan.baidu.com/share/qrcode?w=150&h=150&url=${SSRurl}"
...
View_User_info(){
	...
	复制二维码链接，粘贴到浏览器地址栏就可以看到二维码图片了【本二维码接口由百度免费提供】
	============【小白安全博客：https://blog.xbbai.site】=============
...
Set_config_user(){
	...
	stty erase '^H' && read -p "(默认: xiaobai):" ssr_user
	[[ -z "${ssr_user}" ]] && ssr_user="xiaobai"
...
Set_config_port(){
	...
	stty erase '^H' && read -p "(默认: 9826):" ssr_port
	[[ -z "$ssr_port" ]] && ssr_port="9826"
...
Set_config_password(){
	echo "请输入要设置的用户 密码"
	stty erase '^H' && read -p "(默认: xiaobai):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="xiaobai"
...
if [[ "${action}" == "clearall" ]]; then
	...
	echo -e "  ShadowsocksR 小白一键管理脚本 ${Red_font_prefix}[v${sh_ver}]${Font_color_suffix}
  ---- Barry | https://blog.xbbai.site ----
